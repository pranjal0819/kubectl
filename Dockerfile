...
RUN apt-get update -qq && apt-get install -y curl unzip vim fish sudo less jq
...
RUN if [ "$TARGETPLATFORM" = "linux/arm64" ]; then \
    ...
    curl -L --insecure "https://get.helm.sh/helm-$HELM_VERSION-linux-arm64.tar.gz" | tar -xz && mv linux-arm64/helm /usr/local/bin/helm; \
    else \
